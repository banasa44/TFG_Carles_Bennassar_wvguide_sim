...
# display the array
plt.figure()
plt.title('Transmitance within our laser range wv, for w='+str(Sizes.block_x)+', alpha='+str(Sizes.alpha)+', R='+str(Sizes.R)+'μm')
...
plt.legend(loc="best")
plt.savefig('app/static/images/graph/1350_1450_w_'+str(Sizes.block_x)+'alph_'+str(Sizes.alpha)+', R='+str(Sizes.R)+'μm.png')
plt.show()
...
with open('app/static/csv/trans_full_range_11_R0024_arrays.csv', 'a') as f:
    DF_Ts.to_csv(f,index=True, header=f.tell()==0)
with open('app/static/csv/trans_optim_range_11_R0024_arrays.csv', 'a') as f:
